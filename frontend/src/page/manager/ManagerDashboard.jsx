...
import { Calendar, User, Clock, Filter, CheckCircle, XCircle, Eye, Search, Download } from 'lucide-react';
...
useEffect(() => {
  ...
  const handleStatusUpdate =async (id, newStatus) => {
    try {
        const response=await api.post('/manager/status-Update',{id,newStatus})
        if(response.data.success){
            setSuccessMessage(response.data.message)
            setSuccessModal(true)
          return  setBookings(prev => prev.map(booking => 
  ...
  const getDatas = async () => {
    try {
      ...
      if (response.data.success) {
        const { movies = [], fitness = [], partyHall = [] } = response.data.items;
        ...
        const allBookings = [...transformedMovies, ...transformedFitness, ...transformedParty];
  ...
  const filteredBookings = bookings.filter(booking => {
    ...
    const matchesVoyager = !filters.voyager || booking.voyagerName === filters.voyager;
    const matchesType = !filters.bookingType || booking.bookingType === filters.bookingType;
    const matchesStatus = !filters.status || booking.status === filters.status;

    return matchesSearch && matchesDate && matchesVoyager && matchesType && matchesStatus;
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-header">
        <div className="header-content">
          ...
          <p className="dashboard-subtitle">Manage all booking requests efficiently</p>
        </div>
      ...
      {showFilters && (
        <div className="filter-panel">
          <div className="filter-grid">
            ...
            <div className="filter-group">
              ...
              <select
                ...
                onChange={(e) => setFilters({...filters, voyager: e.target.value})}
                className="filter-select"
              >
                <option value="">All Voyagers</option>
                {uniqueVoyagers.map(voyager => (
                  <option key={voyager} value={voyager}>{voyager}</option>
                ))}
              </select>
            </div>
          ...
          <button 
            ...
            onClick={() => setFilters({date: '', voyager: '', bookingType: '', status: ''})}
          >
            Clear All Filters
          </button>
        </div>
                  ...
                  <td className="user-cell">
                    <div className="user-info">
                      ...
                      <div>
                        ...
                        <div className="voyager-name">{booking.voyagerName}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="booking-cell">
                    <div className="booking-info">
                      <div className="booking-type">
                        <span className="booking-icon">{getBookingIcon(booking.bookingType)}</span>
                        {booking.bookingType}
                      </div>
