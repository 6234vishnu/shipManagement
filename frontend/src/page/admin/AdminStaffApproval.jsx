...
const AdminStaffApproval = () => {
  ...
  const getRequest = async () => {
    try {
      ...
      console.log(
        "error in get request admin side staffs signup approval",
        error
      );
  ...
  const handleApprove = async (staffId) => {
    try {
      const response = await api.post(
        `/admin/staffRequest-Approval?staffId=${staffId}`
      );
      if (response.data.success) {
  ...
  const handleReject = async (staffId) => {
    try {
      const response = await api.post(
        `/admin/staffRequest-Rejection?staffId=${staffId}`
      );
      if (response.data.success) {
  ...
  const filteredRequests = staffRequests.filter((staff) => {
    const matchesSearch = staff.username
      .toLowerCase()
      ...
      <div className="admin-container">
        <div className="admin-header">
          ...
          <p className="admin-subtitle">
            Review and approve pending staff signup requests
          </p>
        </div>
